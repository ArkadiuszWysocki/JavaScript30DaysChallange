  // Get your shorts on - this is an array workout!
    ...
    const inventors2 = inventors.filter(inv => {
      if (inv.year >= 1500 && inv.year < 1600) return inv
    });

    // Array.prototype.map()
    // 2. Give us an array of the inventors first and last names

    const inventors3 = inventors.map(inv => inv.first + " " + inv.last);

    // Array.prototype.sort()
    // 3. Sort the inventors by birthdate, oldest to youngest

    const inventros4 = inventors.sort((a, b) => {
    ...
    const inventors5 = inventors.reduce((a, b) => {
      return a + (b.passed - b.year);
    }, 0);

    // 5. Sort the inventors by years lived

    const inventors6 = inventors.sort((a, b) => {
    ...
    // const mwCategory = document.querySelector('.mw-category');
    ...
    const sumUpInstances = data.reduce((a, index) => {
      if (!a[index]) {
        a[index] = 0;
      }
      a[index]++;
